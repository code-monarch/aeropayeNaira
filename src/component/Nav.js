...
const Nav = ({ userName, isActive, setIsActive }) => {
    ...
    const [showProfile, setShowProfile] = useState(window.matchMedia("(max-width:1260px)").matches);
    const [showNotify, setShowNotify] = useState(window.matchMedia("(max-width:1150px)").matches);
    const [showMobileNav, setShowMobileNav] = useState(window.matchMedia("(max-width:920px)").matches);
  ...
  return (
    <>
        {
            showMobileNav ? <MobileNav isActive={isActive} setIsActive = {setIsActive} /> :
    ...
    <div className='flex items-center '>
        <NavLink to='/' 
        className={`mx-[14px] lg:mx-[23px] navigation-link ${isActive === 'home' &&  'active-nav'}`} 
        ...
        <NavLink to='/flights'  
        className={`mx-[14px] lg:mx-[23px] navigation-link ${isActive === 'flight' &&  'active-nav'}`} 
        ...
        <NavLink to='/wallet'  
        className={`mx-[14px] lg:mx-[23px] navigation-link ${isActive === 'wallet' &&  'active-nav'}`} onClick={() => setIsActive('wallet')}>
                {
                isActive === 'wallet' ? 
        ...
        className={`mx-[14px] lg:mx-[23px] navigation-link ${isActive === 'trans' &&  'active-nav'}`} 
        ...
        <NavLink to='/settings'  
        className={`mx-[14px] lg:mx-[23px] navigation-link ${isActive === 'setting' &&  'active-nav'}`} onClick={() => setIsActive('setting')}>
            {
            isActive === 'setting' ? 
        ...
        <div className='flex items-center justify-between'>
            <button className={`btn-badge ${showNotify ? 'btn-badge-responsive' : ''}`}>
                <span className={`w-[18px] h-[18px] ${showNotify && 'w-[24px] h-[24px]'}`}>
                     <Notification className={`${showNotify && 'w-[24px] h-[24px]'}`} />
                </span>
            ...
            {
                showProfile ? 
                <button className="profile-button relative" onClick={() => setShowOption(!showOption)}>
                    <div className="flex items-center">
                        <Profile /> 
                     </div>

              {
                ...
                <ul className="profile-dropdown absolute top-8 z-[99999] right-0 ">
                  <li className='profile-name flex items-center'><Profile className='mr-[8px]'/> Derek</li>
                  <li>
                    <NavLink to='/passenger' className="settings">Settings</NavLink>
                  </li>
                   <li>
              ...
              <button className="profile-btn relative" onClick={() => setShowOption(!showOption)}>
