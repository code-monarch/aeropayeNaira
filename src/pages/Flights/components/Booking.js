...
const Booking = ( ) => {
  ...
  const [mobileView, setMobileView] = useState(window.matchMedia("(max-width:972px)").matches);
  ...
  return (
    <>
       {
        mobileView ? 
          <MobileBooking childCount={childCount} setAdultCount={setAdultCount} setChildCount={setChildCount} adultCount={adultCount} infantCount={infantCount} setInfantCount={setInfantCount} CabinClass={CabinClass} showTick={showTick} tick={tick} switchText={switchText} departure={departure} arrival={arrival} setDeparture = {setDeparture} setArrival={setArrival} /> : 
        <div className={tripType === 'Round Trip' ? 'booking-container-2' : 'booking-container'}>
            <div className="flex items-center">

              <button className="booking-container-dropdown dropdown-1 flex items-center justify-between"
              onClick={() => tripType === 'One-Way' ? setTripType('Round Trip') : setTripType('One-Way')}>
                <div>
                  {tripType}
                </div>
                <Arrow />
